...
(function () {
    try {
        function lgbt() {
            ...
            lgbt.id = 'lgbt-blur-fullscreen-background';
            lgbt.style = 'background:linear-gradient(180deg,#FE0000 16.66%,#FD8C00 16.66%,33.32%,#FFE500 33.32%,49.98%,#119F0B 49.98%,66.64%,#0644B3 66.64%,83.3%,#C22EDC 83.3%);-webkit-filter:blur(5px);filter:blur(5px);position:fixed;top:0;left:0;width:100%;height:100%;z-index:-1;';
            document.body.appendChild(lgbt);
        };
        function consoleLGBTFlag() {
            // print LGBT flag to console
            console.log('%c%s', 'font-size:10em;', '🏳️‍🌈');
        }
        // June 28th
        if (new Date().getDate() === 28 && new Date().getMonth() === 5) {
            consoleLGBTFlag();
            lgbt();
        }
    } catch (e) {
